...
import 'package:compound/shared_components/data_table/custom_data_table.dart' as ct;
import 'package:compound/shared_components/data_table/custom_data_table_source.dart';
...
class CustomPaginatedDataTable extends StatefulWidget {
  ...
  final List<ct.DataColumn> columns;
...
class CustomPaginatedDataTableState extends State<CustomPaginatedDataTable> {
  ...
  final Map<int, ct.DataRow> _rows = <int, ct.DataRow>{};
  ...
  ct.DataRow _getBlankRowFor(int index) {
    return ct.DataRow.byIndex(
      index: index,
      cells: widget.columns.map<ct.DataCell>((ct.DataColumn column) => ct.DataCell.empty).toList(),
    );
  }

  ct.DataRow _getProgressIndicatorRowFor(int index) {
    bool haveProgressIndicator = false;
    final List<ct.DataCell> cells = widget.columns.map<ct.DataCell>((ct.DataColumn column) {
      if (!column.numeric) {
        haveProgressIndicator = true;
        return const ct.DataCell(CircularProgressIndicator());
      }
      return ct.DataCell.empty;
    }).toList();
    if (!haveProgressIndicator) {
      haveProgressIndicator = true;
      cells[0] = const ct.DataCell(CircularProgressIndicator());
    }
    return ct.DataRow.byIndex(
      index: index,
      cells: cells,
  ...
  List<ct.DataRow> _getRows(int firstRowIndex, int rowsPerPage) {
    final List<ct.DataRow> result = <ct.DataRow>[];
  ...
  Widget build(BuildContext context) {
    ...
    footerWidgets.addAll(<Widget>[
      ...
      Container(width: 32.0),
      IconButton(
        icon: Icon(Icons.chevron_left, color: AppTheme.instance.buttonPrimary),
      ...
      IconButton(
        icon: Icon(Icons.chevron_right, color: AppTheme.instance.buttonPrimary),
        padding: EdgeInsets.zero,
              ...
              DefaultTextStyle(
                ...
                child: IconTheme.merge(
                  ...
                  child: Container(
                    ...
                    color: AppTheme.instance.tableBackground,
                    height: 56.0,
